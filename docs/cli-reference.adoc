...
| link:#debug[debug]
| Debug commands (info, port-forward)
...
odo --alsologtostderr --log_backtrace_at --log_dir --logtostderr --o --stderrthreshold --v --vmodule : odo (OpenShift Do) (app, catalog, component, config, create, debug, delete, describe, link, list, log, login, logout, preference, project, push, service, storage, unlink, update, url, utils, version, watch)
    ...
    component --app --context --project --short : Manage components (create, delete, describe, link, list, log, push, unlink, update, watch)
        ...
        update --app --context --git --local --project --ref --show-log : Update the source code path of a component
    ...
    config : Change or view configuration (set, unset, view)
        set --context --env --force --now : Set a value in odo config file
        unset --context --env --force --now : Unset a value in odo config file
        view --context : View current configuration values
    create --app --binary --context --cpu --env --git --max-cpu --max-memory --memory --min-cpu --min-memory --now --port --project --ref : Create a new component
    debug : Debug commands (info, port-forward)
        info --context : Displays debug info of a component
    ...
    url : Expose component to the outside world (create, delete, list)
        create --context --now --port --secure : Create a URL for a component
        delete --context --force --now : Delete a URL
